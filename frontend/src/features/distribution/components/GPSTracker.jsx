...
export function GPSTracker() {
    ...
    const startTracking = () => {
        ...
        const id = navigator.geolocation.watchPosition(
            async (position) => {
                ...
                const res = await distributionService.updateGPSLocation(latitude, longitude);
                if (res.success) {
    ...
    if (user?.role !== 'DISTRIBUCION' || hideWidget) {
        return null;
    }
        ...
        <>
                    ...
                    <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg shadow-lg flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-yellow-700 mt-1">Activa el GPS para mejor seguimiento de entregas y trazabilidad</p>
                            </div>
                        </div>
                        <button
            ...
            <div className="fixed bottom-4 right-4 bg-white rounded-2xl shadow-2xl border border-gray-200 p-4 max-w-xs z-50">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-bold text-gray-900">🚚 GPS Distribución</h3>
                    <button
                ...
                {locationData && (
                    <div className="space-y-2">
                        <div className="bg-blue-50 rounded-lg p-2">
                            <p className="text-[10px] text-blue-600 font-semibold">Latitud</p>
                            <p className="text-xs text-blue-900 font-mono">{locationData.latitude.toFixed(6)}</p>
                        </div>
                        <div className="bg-blue-50 rounded-lg p-2">
                            <p className="text-[10px] text-blue-600 font-semibold">Longitud</p>
                            <p className="text-xs text-blue-900 font-mono">{locationData.longitude.toFixed(6)}</p>
                        </div>
                        {lastUpdate && (
                ...
                <p className="text-[9px] text-gray-400 text-center mt-3">
                    Tu ubicación se comparte con gerencia en tiempo real
                </p>
            </div>
        </>
    );
